//package PrvKolokviumVezbi.FourthExercise;
//
...
//        LogProcessor<T> firstLogProcessor = (LogProcessor<T>) logs.stream().filter(r -> ((ILog)r).getType().equals("INFO")).collect(Collectors.toList());
//
//        LogProcessor<T>secondLogProcessor = (LogProcessor<T>) logs.stream().filter(r -> ((ILog)r).getMessage().length() < 100).collect(Collectors.toList());
//
//        LogProcessor<T> thirdLogProcessor = (LogProcessor<T>) logs.stream().map;
//
